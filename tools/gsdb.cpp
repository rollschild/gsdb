...
void handle_variable_locals_command(gsdb::target& target) {
    auto pc = target.get_pc_file_address();
    auto scopes = pc.elf_file()->get_dwarf().scopes_at_address(pc);
    // If any variables in nested scopes shadow variables in outer scopes, we
    // should print out only the most nested variable.
    ...
    for (auto& scope : scopes) {
        for (auto& var : scope.children()) {
            ...
            if ((tag == DW_TAG_variable or tag == DW_TAG_formal_parameter) and
                ...
                auto str = gsdb::typed_data{std::move(value), type}.visualize(
                    target.get_process());
                std::print("{}: {}\n", name, str);
                seen.insert(name);
            }
...
void handle_variable_read_command(gsdb::target& target,
                                  const std::vector<std::string>& args) {
    auto name = args[2];
    auto pc = target.get_pc_file_address();
...
void handle_variable_location_command(gsdb::target& target,
                                      const std::vector<std::string>& args) {
    auto name = args[2];
    auto pc = target.get_pc_file_address();
    ...
    auto print_simple_location = [](auto* loc) {
        if (auto reg_loc =
                std::get_if<gsdb::dwarf_expression::register_result>(loc)) {
            auto name = gsdb::register_info_by_dwarf(reg_loc->reg_num).name;
            std::print("Register: {}\n", name);
    ...
    if (auto simple_loc =
        ...
        for (auto& piece : pieces_res->pieces) {
            std::print("Piece: offset = {}, bit size = {}, location = ",
