...
- **`test/`** - Unit tests using Catch2 v3 (`Catch2::Catch2WithMain` supplies `main()`). Test helper binaries (debuggee targets) live in `test/targets/` and are compiled as separate executables. Tests reference them via the `TARGETS_DIR` compile definition, which points to the build-tree location of these binaries. `process::launch(path, false)` launches without tracing (no `PTRACE_TRACEME`), used in tests that separately `attach()`.
