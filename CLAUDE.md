...
- `[elf]` — ELF parsing: symbol lookup by address/name, and load-bias-relative lookup after `notify_loaded()` (uses the `hello_gsdb` target, verifies the `_start` symbol)
- `[dwarf]` — DWARF parsing (uses the `hello_gsdb` target unless noted): root-DIE `DW_AT_language` resolves to `DW_LANG_C_plus_plus`, child-DIE iteration, finding the `main` `DW_TAG_subprogram` by `DW_AT_name`, and `range_list` decoding (including base-address selectors and end-of-list sentinels) over hand-built `.debug_ranges` bytes. The `multi_cu` target (two translation units, `-gdwarf-4`) exercises multi-compile-unit handling.
...
- **ELF parser** (`elf` class in `include/libgsdb/elf.hpp` / `src/elf.cpp`): RAII wrapper over an `mmap`'d ELF file. Constructor `open()`s the file, `fstat`s for size, then `mmap`s `PROT_READ | MAP_SHARED` so the whole file is addressable as a `std::byte*` (`data_`); destructor `munmap`s and `close`s. After mapping it copies the `Elf64_Ehdr` out via `std::copy` + `as_bytes`, calls `parse_section_headers()` (which handles the ≥0xff00-sections sentinel where `e_shnum==0` and the real count lives in section 0's `sh_size`), then `build_section_map()` to populate an `unordered_map<string_view, Elf64_Shdr*>` keyed by section name. The section-name string table is found via `header_.e_shstrndx` (usually `.shstrtab`); `get_section_name(index)` returns a `string_view` constructed from the raw `char*` inside the mmap (the `const char*` constructor uses `strlen` to find the NUL terminator inside the table). `get_string(index)` does the same against `.strtab` (falling back to `.dynstr`). `get_section_contents(name)` returns a `gsdb::span<const std::byte>` viewing the bytes directly inside the mmap. **Lifetime caveat**: all returned `string_view`s and `span`s alias the mmap region — they dangle if the `elf` object is destroyed.
- **ELF symbol tables** (`elf::parse_symbol_table()` / `build_symbol_maps()`): `parse_symbol_table()` copies the `.symtab` entries (falling back to `.dynsym`) into `symbol_table_` (`vector<Elf64_Sym>`). `build_symbol_maps()` then builds two indexes: `symbol_name_map_` (an `unordered_multimap<string_view, Elf64_Sym*>` — multimap because one name can map to several symbols, e.g. local-static vs global, weak vs strong; each symbol is inserted under both its mangled name and, when `__cxa_demangle` succeeds, its demangled name) and `symbol_addr_map_` (a `std::map<pair<file_addr,file_addr>, Elf64_Sym*, range_comparator>` keyed by the `[st_value, st_value+st_size)` range, **sorted only by start address** since `range_comparator` compares `.first` only; symbols with no address/name or `STT_TLS` type are skipped). Lookups: `get_symbols_by_name()` uses `equal_range`; `get_symbol_at_address()` uses `find` for an exact start match; `get_symbol_containing_address()` uses `lower_bound` + a step-back to find the symbol whose range encloses the address. Each address lookup has both a `file_addr` and a `virt_addr` overload (the latter converts via the load bias first).
- **DWARF parser** (`dwarf`/`compile_unit`/`die`/`abbrev`/`attr`/`range_list` in `include/libgsdb/dwarf.hpp` / `src/dwarf.cpp`; raw `DW_*` constant enums in `include/libgsdb/detail/dwarf.h`): a from-scratch reader for the `.debug_info`/`.debug_abbrev`/`.debug_str`/`.debug_ranges` sections of an already-parsed `elf`. **Wired into `elf`**: each `elf` lazily owns a `std::unique_ptr<dwarf>` exposed via `elf::get_dwarf()`, and the `[dwarf]` tests exercise it. Constructed as `dwarf(const elf&)`; the ctor eagerly calls `parse_compile_units()` (walks `.debug_info`, one CU header at a time). **Scope limited to DWARF32, version 4, 8-byte addresses** — anything else throws via `error::send` (note GCC emits DWARF4 by default even for C++; some DWARF5 attribute constants like `DW_AT_defaulted` are included because of this). All parsing goes through an internal anonymous-namespace `cursor` (in `dwarf.cpp`) that walks a `span<const std::byte>` into the mmap and decodes fixed-width ints, NUL-terminated strings, **ULEB128/SLEB128** (`uleb128()`/`sleb128()`), and — crucially — `skip_form(DW_FORM_*)` to advance past an attribute value of any form without decoding it. Abbreviation tables are parsed lazily and cached: `dwarf::get_abbrev_table(offset)` parses `.debug_abbrev` at `offset` on first request and memoizes into `abbrev_tables_` (keyed by section offset); each `compile_unit` holds its `abbrev_offset_` and forwards `abbrev_table()` to the parent. A `compile_unit` owns its slice of `.debug_info` (`data_`) and exposes `root()` (skips the fixed 11-byte CU header, then `parse_die`). A `die` stores its position, owning CU, resolved `abbrev*`, the `next_` pointer (start of the following DIE), and a precomputed `attr_locs_` vector (one byte-pointer per attribute, so any attribute can be decoded later without re-walking). DIE tree traversal is via `die::children()` → `children_range` with a `forward_iterator` (`die::children_range::iterator`): a DIE with no children yields `end()`; `operator++` advances to the next sibling, **recursively skipping over a child subtree** by constructing a sub-iterator and walking it to its null terminator before reading the sibling. A null DIE (abbrev code 0) marks end-of-siblings and is detected by `abbrev_entry() == nullptr`.
  - **Attribute access**: `die::contains(DW_AT_*)` tests presence; `die::operator[](DW_AT_*)` returns an `attr` (which carries the owning CU, the `DW_AT_*` name, the `DW_FORM_*` form, and a byte-pointer into the mmap). `attr` decodes lazily via typed accessors — `as_int`, `as_string` (resolves `DW_FORM_strp` against `.debug_str`), `as_address`, `as_section_offset`, `as_block`, `as_reference` (follows a CU-relative DIE offset and returns the referenced `die`), and `as_range_list` (returns a `range_list` over `.debug_ranges`). Convenience helpers on `die`: `name()`, `low_pc()`, `high_pc()`, and `contains_address(file_addr)` (checks `DW_AT_ranges` if present, else the `[low_pc, high_pc)` interval).
  - **Range lists**: `range_list` iterates `.debug_ranges` entries (`forward_iterator`), handling **base-address selectors** (`~0ULL` low value resets the running base) and the all-zero end-of-list sentinel; each yielded `entry{low, high}` has its own `contains()`, and `range_list::contains(addr)` scans them.
  - **Address/function lookup & indexing**: `dwarf::compile_unit_containing_address()`, `function_containing_address()`, and `find_functions(name)`. These are backed by a lazily-built, `mutable` `function_index_` (`unordered_multimap<string, index_entry>` mapping function names → `{cu, die-position}`), populated by `index()`/`index_die()` which walk every DIE once on first use.
  - **Lifetime caveat**: like the rest of the ELF layer, every `span`/`string_view`/byte-pointer aliases the `elf`'s mmap and the owning `dwarf`/`compile_unit` objects — they dangle if those are destroyed.
- **Address types** (`types.hpp` / `types.cpp`): three strong types distinguish the kinds of address the debugger juggles. `virt_addr` — a real runtime virtual address in the inferior. `file_addr` — a virtual address *as written in the ELF file*; it carries a `const elf*` so comparisons assert both sides belong to the same object. `file_offset` — a raw byte offset from the start of the file. The file/runtime split differs by a single **load bias** for the whole object. Conversions live in `types.cpp`: `file_addr::to_virt_addr()` adds `elf::load_bias()`, `virt_addr::to_file_addr(elf&)` subtracts it; both first call `get_section_containing_address()` to validate the address belongs to a real section (returning a default-constructed address otherwise).
- **Load bias & `target`**: the load bias is discovered at runtime, not from the file. `process::get_auxv()` reads `/proc/<pid>/auxv` into an `unordered_map<int, uint64_t>` (keyed by `AT_*` macros). The `target` class (`include/libgsdb/target.hpp` / `src/target.cpp`) is the higher-level coordinator that owns a `process` *and* its `elf` — same factory pattern as `process` (deleted default/copy, private constructor, static `target::launch(path)` / `target::attach(pid)` returning `unique_ptr`). Its helper `create_loaded_elf()` builds the `elf`, then calls `elf::notify_loaded(virt_addr{auxv[AT_ENTRY] - e_entry})` to set the load bias by subtracting the ELF header's entry point from the *actual* entry point the kernel reports. `attach()` resolves the binary path from the `/proc/<pid>/exe` symlink.
- **Utility headers**: `bit.hpp` provides `from_bytes<T>`, `as_bytes`, `to_byte128/64` for safe type-punning via `memcpy`. `parse.hpp` provides `to_integral<T>`, `to_float<T>`, and `parse_vector<N>` for parsing user input (register values). `types.hpp` defines the strong address types (`virt_addr`, `file_addr`, `file_offset` — see the "Address types" pattern below), `byte64`/`byte128` aliases, the `stoppoint_mode` enum, and a minimal `span<T>` template (data pointer + size, with `begin`/`end`/`operator[]` and a `vector<U>` converting constructor) used by the ELF parser to hand back views into the mmap.
